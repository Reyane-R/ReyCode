defmodule Breeze.ChildServer do
  ...
  defp process_input(event, term, opts) do
    if function_exported?(term.view, :intercept_input, 2) do
      case term.view.intercept_input(event, term) do
        {:cont, next_term} -> route_input(event, next_term, opts)
        {:cont, replacement_event, next_term} -> route_input(replacement_event, next_term, opts)
        {:halt, next_term} -> {:noreply, next_term}
      end
    else
      route_input(event, term, opts)
    end
  end

  defp route_input(%{"key" => key} = event, term, opts) when key in ["\t", "Tab"] do
    event
    |> Breeze.InputCapture.normalize_focus_key()
    |> process_input(term, opts)
  end

  defp route_input("\t", term, opts) do
  ...
  defp route_input("ShiftTab", term, opts) do
  ...
  defp route_input(%{"mouse" => mouse} = event, term, opts) do
  ...
  defp route_input(key, term, opts) do
