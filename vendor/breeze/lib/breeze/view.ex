defmodule Breeze.View do
  ...
  * `focus_scope: :trap` - constrain tab/shift-tab navigation to this implicit subtree
  ...
  @doc "Intercepts input before child routing. Halt captures the event; cont preserves normal routing."
  @callback intercept_input(term(), Breeze.Term.t()) ::
              {:cont, Breeze.Term.t()}
              | {:cont, term(), Breeze.Term.t()}
              | {:halt, Breeze.Term.t()}

  @optional_callbacks mount: 2, render: 1, handle_event: 3, handle_info: 2, intercept_input: 2

  @doc "Returns the last committed layout for named elements, in receiving-view coordinates."
  def element_layouts(term), do: term.elements

  @doc "Returns the rendered vertical scroll offset of a named element."
  def scroll_offset_y(term, id) do
    case Map.get(term.rendered_boxes, id) do
      %BackBreeze.Box{scroll: {_x, y}} -> y
      _ -> 0
    end
  end
